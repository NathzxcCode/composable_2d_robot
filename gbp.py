...
from gbp_utilities import Gaussian, MeasModel 
...
class FactorGraph:
    ...
    def add_var_node(self,
        ...
        new_node = VariableNode(id, dofs, properties=properties)
        if prior_mean is not None and prior_diag_cov is not None:
            ...
            new_node.prior.set_with_cov_form(prior_mean, prior_cov)
            new_node.update_belief()

        if self.var_nodes.get(id) == None:
            self.var_nodes[id] = [new_node]
        else:
